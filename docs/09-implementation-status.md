...
Screen recording starts only after the creator chooses a window, application, or display in the system ScreenCaptureKit picker. The adapter can include system/application audio and microphone audio, keeps those sources separate, offers configurable cursor visibility and click highlights, records opted-in pointer motion only for the active local session, excludes Creator Studio's own audio, and caps capture dimensions to an even, aspect-preserving 4K working size. The app is sandboxed, uses a sandbox-owned recovery inbox, and has no network dependency.

ScreenCaptureKit sample buffers feed the same bounded `CaptureWriterPipeline` used by the ReplayKit foundation. Screen, application audio, and microphone are written as independent ten-second H.264/AAC segments. Every committed segment is hashed and journaled before the manifest replacement. A normal stop force-reads its finalized manifest, creates a timestamped recording name, imports immutable project-owned sources into the reusable **Unlisted Recordings** project, and removes only the now-redundant recovery-inbox staging copy. Interrupted, failed, and storage-constrained sessions remain visible for explicit recovery or deletion; ordinary completed takes never require a save click.

The desktop library presents a Local Workspace → Projects → Recordings hierarchy. Each project has visible add and delete controls: add opens the safe file importer for a new immutable recording source, while delete asks for confirmation before removing that project package. Library summaries group one capture session's screen and audio assets as one recording without another disk read. Opening a project no longer starts cache generation for every source; proxy and waveform work begins only for the recording being previewed.

Still pending on macOS: signed/runtime Screen Recording and microphone qualification; A/V sync, long-capture, protected-content, thermal, storage, and multi-display measurement; richer editable pointer paths; camera capture; external-drive handoff; and direct-manipulation precision editing. An unsigned SDK build proves compilation and package integration but does not prove TCC permission behavior or real ScreenCaptureKit media delivery.
...
The desktop now separates the source-focused Media tab from an Editor tab. The Editor explains the master timeline, gives clips their own inspector, supports trim, split, move, hide, delete, persisted undo/redo, and can merge another saved recording at the end of every matching timeline track without modifying the original. Click-derived and explicit pointer focus markers become persisted, undoable focus events, and exports render their comfortable zoom-in/hold/zoom-out transforms in a 1080p landscape, vertical, or square QuickTime movie through the existing `RenderPlan` and export-preflight boundary. The cache control clears only rebuildable project cache products; project deletion removes that project package, while recovery deletion removes only the selected inbox session.

The macOS menu bar now exposes global `⌘⇧R` start/stop recording and `⌘⇧F` focus-at-cursor commands, alongside cursor, click-highlight, pointer-tracking, and automatic-focus controls. It also exposes import, merge, cache, export, undo/redo, media/editor, and playback actions for the selected project. Preview uses the supported `AVPlayerView` QuickTime-style control surface with sharing, frame stepping, timecodes, full-screen, Picture in Picture, and standard playback shortcuts; it does not attempt to embed the separate QuickTime Player application.
...
- Swift package suite covers the capture state machine, low-storage and capability seams, atomic manifest/journal replay, forced finalized-manifest discovery, injected interruption, traversal, symlink, hash mismatch, partial-import reporting, immutable copy behavior, reader/writer schema compatibility, identifier drift between Swift and the project generator, deterministic timeline commands, focus-event undo/redo, multi-session Unlisted Recordings append behavior, bounded history, persisted undo/redo, synchronized capture-track bootstrapping, media metadata persistence, real AVFoundation audio/video fixture inspection, proxy export, waveform reuse, recovery-session deletion, cache cancellation and cache clearing, and master-timeline merge persistence.
